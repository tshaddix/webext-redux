...
export default (
  ...
  console.clear();
  console.log("debug wrapStore start");
  if (!portName) {
  ...
  const connectState = (port) => {
    console.log("debug connectState start");
    ...
    console.log("debug before initial state send");
    // Send store's initial state through port
    serializedMessagePoster({
      type: STATE_TYPE,
      payload: prevState,
    });
    console.log("debug after initial state send");
  };
  ...
  console.log(
    "debug browserAPI.runtime.onConnect.addListener",
  ...
  console.log("debug before safety");
  /**
  ...
  console.log("debug wrapStore finish");
